...
function largestProductinaSeries(n) {
  ...
  let max = 0;
  // Goes through every digit (as starting point) until the nth-last digit
  for (let start = 0; start <= 1000 - n; start++) {
    let product = 1;
    // Calculate the product of the adjacent n digits from "start"
    for (let i = start; i < start + n; i++) {
        product *= parseInt(thousandDigits[i]);
    }
